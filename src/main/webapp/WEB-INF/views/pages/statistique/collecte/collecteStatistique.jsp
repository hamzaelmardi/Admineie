...
<script>
    ...
    function getNombreRegionCT(reg){
        ...
        $(".pie1").hide();
        $.ajax({
            url:"/api/statistique/getCountCollectByRegion",
            type:"GET",
    ...
    function getSocieteRegionCT(reg){
        ...
        $(".pie1").hide();
        $.ajax({
            url:"/api/statistique/getCollectByRegion",
            type:"GET",
            ...
            if(prefecture==0){
                ...
                $(data).each(function(idx,el){
                    ...
                    tab1 += '<ul id="chart-selector"> <button class="btn btn-outline-success" onclick="tableToExcel(\'tab_stat\', \'Statistique des autorisations\')"><span style="color:#1b5e20"><i class="fa fa-file-excel"></i> Exporter EXCEL</span></button> </ul>' +
                        '<div  id="tab_stat"><p class="text-center" style="color: #003399; font-size: 20px; font-family: sans-serif !important; font-weight: bold">Les sociétés autorisées à collecter et transporter les Déchets Dangereux </p> ' + tr2 + '</div>';
                    $("#resultat").html(tab1);
