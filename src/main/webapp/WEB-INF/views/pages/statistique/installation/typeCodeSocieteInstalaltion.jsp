...
<script>
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            url:"/api/getSocietesInstallation",
            type:"GET",
            data:{
            },
    ...
    function getCodeTypeBySociete(societe) {
        ...
        $.ajax({
            url:"/api/statistique/getTypeCodeSocieteInstalaltion",
            type:"GET",
            ...
            tab1 += '<ul id="chart-selector"> <button class="btn btn-outline-success" onclick="tableToExcel(\'tab_stat\', \'Statistique des autorisations\')"><span style="color:#1b5e20"><i class="fa fa-file-excel"></i> Exporter EXCEL</span></button> </ul>' +
                '<div  id="tab_stat"><p class="text-center" style="color: #003399; font-size: 20px; font-family: sans-serif !important; font-weight: bold">Les sociétés autorisées à traiter les Déchets Dangereux par Type des déchets  </p> ' + tr2 + '</div>';
            $("#resultat").html(tab1);
