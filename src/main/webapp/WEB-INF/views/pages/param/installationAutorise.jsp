    ...
    <div class="col-12 from_add" id="frm_etatDoss" style="display: none; margin-top: 43px; padding-right: 20%; float: none;">
        <div class="row p-0 m-0 justify-content-center">
            <div class="col-xl-5 col-lg-7 mt-5 col-sm-12 mt-5 grid-margin" >
                ...
                <form name="installation" id="installation">
                <div class="card" >
                        ...
                        <div class="col-10 mt-4">
                            <h3 class="ml6 float-right mb-1">
                                <span class="text-wrapper">
                                    <span class="letters"><span class="letters"><spring:message code="label.InstallationAutorise" /></span></span>
                                </span>
                            </h3>
                            <div class="row p-0 m-0 mt-5"></div>
                            <div class="row p-0">
                                <div class="col-12 mt-4">
                                    <div class="form-group">
                                        <label><spring:message code="label.InstallationAutorise" /></label>
                                        <input type="hidden" id="id_installation_autorise" value="${installationAutorise.id_installation_autorise}" class="form-control">
                                        <input type="text" id="raison"  value="${installationAutorise.raison}" class="form-control">
                                    </div>
                                </div>
                                <div class="col-12 mt-4">
                                    <div class="form-group">
                                        <label><spring:message code="label.Tel" /></label>
                                        <input type="text" id="tel"  value="${installationAutorise.tel}" class="form-control">
                                    </div>
                                </div>
                            </div>
                            <div class="row p-0">
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="fax" value="${installationAutorise.fax}" class="form-control">
                                    </div>
                                </div>
                                <div class="col-12">
                                    <div class="form-group">
                                        <label><spring:message code="label.province"/> </label>
                                        <select type="text" id="prefecture"  class="form-control">
                                            <option value="${installationAutorise.prefecture.id_prefecture}">${installationAutorise.prefecture.nom_fr}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row  mb-4" style="margin:14px;">
                        <a  class="btn btn-primary btn-block " onclick="addInstallation()" ><spring:message code="label.Enregistrer"/> </a>
                        <a class="btn btn-danger-2 text-white btn-block" onclick="getFunctionParam('getParamInstallation/0/6')"><spring:message code="label.Annuler"/></a>
                    </div>
                </div>
                </form>
            </div>
                    ...
                    <div class="row m-0">
                        ...
                        <div class="col-9">
                            <a class="button text-white fr_btn" onclick="frm_table('subtab_0','frm_etatDoss')">
                                <span><spring:message code="label.NouvelleInstallation"/> </span>
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div class="row justify-content-center">
                <div class="col-12">
                    <table id="example2" class="table tablee table-hover">
                        <thead>
                        <tr>
                            <th class="text-center"><spring:message code="label.Societe"/></th>
                            <th class="text-center"><spring:message code="label.ville"/> </th>
                            <th class="text-center">Date de délivrance</th>
                            <th class="text-center">Prorogation</th>
                        ...
                        <c:forEach items="${listF}" var="f">
                            <tr id="tr-${f.id_installation_autorise}">
                                <td>${f.raison}<br/>${f.tel}<br/>${f.fax}</td>
                                <td>${f.prefecture.nom_fr}</td>
                                <td><fmt:formatDate pattern="dd MMMM yyyy" dateStyle="long" value="${f.date_delivrance }"/></td>
                                <td><fmt:formatDate pattern="dd MMMM yyyy" dateStyle="long" value="${f.date_prorogation }"/></td>
                                <td><fmt:formatDate pattern="dd MMMM yyyy" dateStyle="long" value="${f.date_fin }"/></td>
                                <td class="text-center">
                                    <ul class="list-inline m-0">
                                        <li class="list-inline-item">
                                            <button onclick="updateInstallation(${f.id_installation_autorise})" class="btn btn-outline-warning btn-sm rounded-circle tab_edit" type="button" title="Editer" style="background-color: white">
                                        ...
                                        <li class="list-inline-item">
                                            <button onclick="delete_Installation(${f.id_installation_autorise})" class="btn btn-outline-danger btn-sm rounded-circle tab_trash" type="button" title="Supprimer" style="background-color: white">
                                                <div class="icon_trash_1">
                        ...
                        <c:when test="${listF!='[]'}">
                            <div class="text-center" dir="ltr">
                                ...
                                <c:if test="${totalPage>0}">
                                    <select class="custom-select" id="optionselected" onchange="getParamInstallation(this.value)" style="margin: auto; width: 100px" >
...
<script type="text/javascript">
    function addInstallation(){
        ...
        var id_installation_autorise=$("#id_installation_autorise").val();
        var tel=$("#tel").val();
        var fax=$("#fax").val();
        var prefecture=$("#prefecture").val();
        var Installation={
            "raison":raison,
            "id_installation_autorise":id_installation_autorise,
            "tel":tel,
            "fax":fax,
        ...
        $.ajax({
            type: "POST",
            url :"/api/addInstallation",
            contentType : 'application/json; charset=utf-8',
            data : JSON.stringify(Installation),
            success : function(response) {
                alert(response);
                $("#tr-"+id_installation_autorise).css("display","block");
